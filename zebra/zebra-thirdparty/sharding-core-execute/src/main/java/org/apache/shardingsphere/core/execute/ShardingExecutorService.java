...
public final class ShardingExecutorService {
    ...
    private ExecutorService getExecutorService(final int executorSize, final String nameFormat) {
        ...
        ExecutorService originalExecutorService =
                0 == executorSize ? Executors.newCachedThreadPool(shardingThreadFactory) : Executors.newFixedThreadPool(executorSize, shardingThreadFactory);

        // zebra定制点 : 添加alibaba的TTL修饰，以便做Tracing
        // tracing需要用到traceId，JDK自带的ThreadLocal*在持线程池下会失效，必须使用TTL才能传递
        return TtlExecutors.getTtlExecutorService(originalExecutorService);
    }
