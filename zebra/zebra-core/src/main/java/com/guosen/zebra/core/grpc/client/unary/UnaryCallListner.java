...
public class UnaryCallListner<Request, Response> extends ClientCall.Listener<Response> implements Runnable {

	private static final Logger log = LogManager.getLogger(UnaryCallListner.class);
	...
	public UnaryCallListner(final GrpcRequest request,final Integer retriesOptions, 
	...
	public void onClose(Status status, Metadata trailers) {
		try {
			...
			if(log.isDebugEnabled()){
				MethodDescriptor<Message, Message> methodDesc = this.orgRequest.getJsonMethodDesc() == null
						? this.orgRequest.getMethodDescriptor() : this.orgRequest.getJsonMethodDesc();
				log.debug("rpc service is {}, response is {},request is {}",methodDesc.getFullMethodName(), response,
						orgRequest.getRequestParam());
			}
			if (status.isOk()) {
				statusOk(trailers);
			} else {
